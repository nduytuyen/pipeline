module pipelined(
    ...
    ID_EX REG_ID_EX (
        ...
        .i_rd_addr(instr_id[11:7]), .i_rs1_addr(instr_id[19:15]), .i_rs2_addr(instr_id[24:20]),
        .i_func3(instr_id[14:12]),
        .i_rd_wren(rd_wren_id), .i_mem_wren(mem_wren_id), .i_mem_rden(mem_rden_id),
        .i_opa_sel(opa_sel_id), .i_opb_sel(opb_sel_id),
        ...
        .o_rd_addr(rd_addr_ex), .o_rs1_addr(rs1_addr_ex), .o_rs2_addr(rs2_addr_ex),
        .o_func3(func3_ex),
        
        .o_rd_wren(rd_wren_ex), .o_mem_wren(mem_wren_ex), .o_mem_rden(mem_rden_ex),
        .o_opa_sel(opa_sel_ex), .o_opb_sel(opb_sel_ex),
    ...
    logic [31:0] alu_data_mem; // Khai báo trước để FWD dùng
    ...
    assign rs1_fwd = (forward_a == 2'b10) ? alu_data_mem : 
                     (forward_a == 2'b01) ? wb_data_wb   : rs1_data_ex;

    assign rs2_fwd = (forward_b == 2'b10) ? alu_data_mem : 
                     (forward_b == 2'b01) ? wb_data_wb   : rs2_data_ex;

    // Dùng Mux cho đầu vào ALU (Đã đổi rs1_data_ex thành rs1_fwd)
    mux_2to1 MUX_OPA (.i_data_0(rs1_fwd), .i_data_1(pc_ex), .i_sel(opa_sel_ex), .o_data(opa));
    mux_2to1 MUX_OPB (.i_data_0(rs2_fwd), .i_data_1(imm_ex), .i_sel(opb_sel_ex), .o_data(opb));

    alu ALU_UNIT (.i_operand_a(opa), .i_operand_b(opb), .i_alu_op(alu_op_ex), .o_alu_data(alu_data_ex));
    
    // Dùng rs1_fwd và rs2_fwd cho bộ so sánh nhánh
    brc BRC_UNIT (.i_br_un(br_un_ex), .i_rs1_data(rs1_fwd), .i_rs2_data(rs2_fwd), .o_br_less(br_less), .o_br_equal(br_equal));
    ...
    logic [31:0] rs2_data_mem, pc_mem;
    ...
    EX_MEM REG_EX_MEM (
        ...
        .i_alu_data(alu_data_ex), .i_rs2_data(rs2_fwd), .i_pc(pc_ex), // LƯU rs2_fwd VÀO MEM CHO LỆNH STORE
    ...
    pc_plus4 PC_FOUR_WB (.pc(pc_wb), .pc_four(pc_four_wb));
